...
import com.apiNft.NFTAPI.web.dto.ResponseComentarioNftDTO;
...
public class CommentController {
    ...
    public ResponseEntity<List<ResponseComentarioNftDTO>> findAllNfts(@PathVariable Long id) {
        ...
        List<ResponseComentarioNftDTO> response = commentsList.stream()
                .map((comment -> new ResponseComentarioNftDTO(
                        comment.getId(),
                        comment.getUsuario().getId(),
                        comment.getUsuario().getUsername(),
                        comment.getMensagem(),
                        "https://img.com",
                        comment.getDate())))
                .toList();
